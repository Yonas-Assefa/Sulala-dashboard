...
export const SafeParseJSON = (jsonString: unknown) => {
  try {
...
export const makeRequestWithCookie = async (
  url: string,
  data: object,
  method: TRequestMethod,
  cookie: string,
...
export const setBrowserCookie = (response: Response) => {
  ...
  for (const cookie of cookieArray) {
    ...
    const path = cookieProps
      .find((prop) => prop.includes("Path"))
      ?.split("=")[1];
    const expires = cookieProps
      .find((prop) => prop.includes("expires"))
      ?.split("=")[1];
    const samesite = cookieProps
      .find((prop) => prop.includes("SameSite"))
      ?.split("=")[1];
...
export const getResponseErrorMessage = (
  ...
  if (body[Object.keys(body)[0]]) {
    if (typeof body[Object.keys(body)[0]] === "string")
      return defaultMessage || "Failed to submit form";
    return (
      body[Object.keys(body)[0]][0] || defaultMessage || "Failed to submit form"
    );
  }
  return defaultMessage || "Failed to submit form";
};
