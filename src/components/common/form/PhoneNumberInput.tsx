...
function PhoneNumberInput({
  ...
  const initialCountryCode = countries.find(
    (country: CountryCode) =>
      country.dial_code === initialValue.countryPhoneCode,
  );
  ...
  React.useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      if (
        ...
        !countryCodeRef.current.contains(e.target as Node)
      ) {
    ...
    if (!initialCountryCode) {
      getUserLocation().then((data) => {
        const country = countries.find(
          (country: CountryCode) => country.code === data.country_code,
        );
  ...
  return (
    <div ref={ref} className="flex flex-col gap-3 w-full">
      <label htmlFor="phone-number" className="self-start">
              ...
              <ul className="max-h-[300px] overflow-y-scroll z-[1]">
                {countries
                  ...
                  .map((country) => (
                    ...
                    >
                      <div
                        className="hover:bg-transparent focus:bg-transparent active:bg-transparent w-full flex justify-between tooltip"
