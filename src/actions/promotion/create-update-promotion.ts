  ...
  try {
    ...
    if (promotion_type == "DISCOUNT") {
      ...
      if (promotional_discount_type === "PERCENTAGE_OFF") {
        ...
        promotional_discount_type == "PERCENTAGE_OFF_THE_MINIMUM_CART_SIZE"
      ) {
        Object.assign(dataToBeParsed, {
          discount: +(formData.get("discount") || 0),
      ...
      if (item_type == "product") {
        Object.assign(dataToBeParsed, {
          products: formData.getAll("product")?.map((product) => +product),
        });
      } else {
        Object.assign(dataToBeParsed, {
          services: formData.getAll("service")?.map((service) => +service),
        });
      }
      ...
      if (destination_type == "LIST_OF_PRODUCTS") {
        ...
        if (item_type == "product") {
          Object.assign(dataToBeParsed, {
            products: formData.getAll("products")?.map((product) => +product),
          });
        } else {
          Object.assign(dataToBeParsed, {
            services: formData.getAll("services")?.map((service) => +service),
          });
        }
      }
    }
