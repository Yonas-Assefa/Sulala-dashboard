...
function ExportProductsModal({ exportData }: Props) {
    ...
    const downloadCsv = () => {
        ...
        pushSuccessNotification(`${t('downloaded_csv_file')} ${csvFileName}.csv`)
        ...
        <dialog id="export_products_modal" className='modal'>
            <div className="modal-box w-11/12 max-w-sm bg-white px-0">
                <div className='border-b-2 border-gray-200 pb-3'>
                    <h3 className="font-bold text-xl text-black text-center font-serif">{t('export_to_csv')}</h3>
                </div>
                <div className="px-5 flex flex-col gap-3 mt-4">
                    <TextInput label={t('file_name')} value={csvFileName} setValue={setCsvFileName} defaultValue={csvFileName} placeholder='example.csv' />
                    <PrimaryButton disabled={!(csvFileName?.length > 0)} name={t('export')} handleClick={downloadCsv} />
                </div>
            </div>
