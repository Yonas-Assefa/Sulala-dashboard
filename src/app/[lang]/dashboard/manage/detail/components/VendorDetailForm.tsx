        ...
        <div className='w-full flex flex-col md:grid md:grid-cols-3 gap-3'>
            <div className='bg-tertiary col-span-2 p-8 rounded-[40px] flex flex-col gap-3'>
                ...
                <div className='max-w-[1300px] grid grid-cols-1 md:grid-cols-2 gap-3'>
                    ...
                    <TextInput defaultValue={initialData?.vendor_name} disabled label={t('vendor_name')} name='name' />
                    <TextInput defaultValue={initialData?.vendor_email} disabled label={t('vendor_email')} name='email' type='email' />
                    <TextInput defaultValue={initialData?.phone_number} disabled label={t('vendor_phone_number')} name='phone number' />
                </div>
            </div>
            <div className='bg-tertiary p-8 rounded-[40px] flex flex-col gap-3'>
                ...
                <div className='max-w-[1300px] grid grid-cols-1 gap-3'>
                    <TextInput defaultValue={initialData?.shop_name} disabled label={t('vendor_shop_name')} name='shop_name' />
                    <TextInput defaultValue={initialData?.shop_category} disabled label={t('vendor_shop_categories')} name='category' />
                    <DateInput defaultValue={initialData?.date} disabled label={t('vendor_shop_registration_date')} name='registration_date' />
                    <TextInput defaultValue={initialData?.shop_address} disabled label={t('vendor_address')} name='address' />
                </div>
            </div>
            <div className='bg-tertiary col-span-2 p-8 rounded-[40px] flex flex-col gap-3'>
                <h3 className='font-semibold text-xl'>{t('vendor_files')}</h3>
                <div className='flex flex-row gap-4'>
                    <button className={file == 'tax' ? 'font-semibold bg-primary text-white p-1 px-3 rounded-md' : 'font-semibold bg-secondary text-white p-1 px-3 rounded-md'} onClick={() => setfile('tax')}>{t('tax_file')}</button>
                    <button className={file == 'cert' ? 'font-semibold bg-primary text-white p-1 px-3 rounded-md' : 'font-semibold bg-secondary text-white p-1 px-3 rounded-md'} onClick={() => setfile('cert')}>{t('certificate_file')}</button>
                </div>
                    ...
                    <div className='flex flex-col gap-3'>
                        <div className='flex flex-col gap-3'>
                            <div className='flex flex-row gap-1'>
                                ...
                                <label htmlFor="approve" className={`cursor-pointer ${status == 'APPROVE' && 'font-semibold text-warning drop-shadow-md'}`}>{t('approve')}</label>
                            </div>
                            <div className='flex flex-row gap-1'>
                                ...
                                <label htmlFor="reject" className={`cursor-pointer ${status == 'REJECT' && 'font-semibold text-error drop-shadow-md'}`}>{t('reject')}</label>
                            </div>
                            {formState?.fieldErrors?.status?.[0] && <span className="text-xs text-danger">
                                {formState?.fieldErrors?.status?.[0]}
                            </span>}
                        </div>
                        {status == 'REJECT' && <TextAreaInput label={t('reason')} placeholder={t('type_here')} name='reason' id='reason' required error={formState?.fieldErrors?.reason?.[0]} />}
                    </div>
                    <div className=''>
