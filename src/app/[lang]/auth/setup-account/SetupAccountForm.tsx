...
function SetupAccountStageOne({ formState, show, personalInfo }: { formState: FormState, show: boolean, personalInfo: any }) {
    ...
    return (
        <section className={`flex flex-col gap-5 w-full items-stretch ${!show && 'hidden'}`}>
            <TextInput defaultValue={first_name} value={first_name} setValue={setFirstName} label={t('what_is_your_first_name')} placeholder={t('enter_your_first_name')} id='first_name' name='first_name' error={formState.fieldErrors?.first_name?.[0]} />
            <TextInput defaultValue={last_name} value={last_name} setValue={setLastName} label={t('what_is_your_last_name')} placeholder={t('enter_your_last_name')} id='last_name' name='last_name' error={formState.fieldErrors?.last_name?.[0]} />
            <TextInput defaultValue={personalInfo?.email || email} disabled={personalInfo?.email?.length > 0} value={email} setValue={setEmail} label={t('what_is_your_email_address')} placeholder={t('enter_your_email_address')} id='email' name='email' error={formState.fieldErrors?.email?.[0]} />
        </section>
    )
}
...
function SetupAccountStageThree({ formState, show }: { formState: FormState, show: boolean }) {
    ...
    return (
        <section className={`flex flex-col gap-5 w-full items-start ${!show && 'hidden'}`}>
            ...
            <FileInput label={t('please_attach_the_certificates')} handleFile={handleCertificate} file={certificate} accept={['.pdf', 'image/*']} id='certificate' name='certificate' error={formState.fieldErrors?.certificates?.[0]} />
            <FileInput label={t('please_attach_the_tax_section')} handleFile={handleTaxForm} file={taxForm} accept={['.pdf', 'image/*']} id='tax_form' name='tax_form' error={formState.fieldErrors?.tax_forms?.[0]} />
        </section>
    )
}
