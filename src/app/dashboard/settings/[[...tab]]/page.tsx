    ...
    return (
        <>
            ...
            <div className='text-black flex flex-col w-full h-full p-8 gap-10 overflow-y-scroll'>
                <h1 className='text-3xl md:text-5xl font-semibold font-serif'>Settings</h1>

                <div className='box-content md:border-b-2 md:border-secondary'>
                    <div className="md:self-start font-medium flex flex-row">
                        <Link href={'/dashboard/settings/personal-info'} className={`tab md:border-b-2 text-xs md:text-md md:px-6 -mb-[1px] ${activeTab == 'personal-info' ? 'md:text-primary md:border-primary text-tertiary bg-primary md:bg-transparent' : 'md:text-secondary md:border-transparent text-primary bg-tertiary md:bg-transparent'}`}>Personal Info</Link>
                        <Link href={'/dashboard/settings/shop-info'} className={`tab md:border-b-2 text-xs md:text-md md:px-6 -mb-[1px] ${activeTab == 'shop-info' ? 'md:text-primary md:border-primary text-tertiary bg-primary md:bg-transparent' : 'md:text-secondary md:border-transparent text-primary bg-tertiary md:bg-transparent'}`}>Shop Info</Link>
                        <Link href={'/dashboard/settings/billing-info'} className={`tab md:border-b-2 text-xs md:text-md md:px-6 -mb-[1px] ${activeTab == 'billing-info' ? 'md:text-primary md:border-primary text-tertiary bg-primary md:bg-transparent' : 'md:text-secondary border-transparent text-primary bg-tertiary md:bg-transparent'}`}>Billing Info</Link>
                    </div>
                </div>
